...
void place_word(slot_t b, const char *word, int word_used_index, int slot_row);
...
int solve_crossword_util(int n, int slot_row)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (1 == is_safe_to_put_word(g_slot_tab[slot_row], &g_input_words[i][0], i, slot_row))
        {
            place_word(g_slot_tab[slot_row], &g_input_words[i][0], i, slot_row);
...
int is_safe_to_put_word(slot_t b, const char *word, int word_used_index, int slot_row)
{
    if (b.s.row == b.e.row)
    {
        if (b.e.col - b.s.col + 1 != strlen(word) || g_input_word_used[word_used_index] == 1)
        {
            return 0;
        }
    }
    ...
    {
        if (b.e.row - b.s.row + 1 != strlen(word) || g_input_word_used[word_used_index] == 1)
        {
            return 0;
        }
    }
    ...
    {
        int ret = slot_cross_check(g_slot_tab[row], g_slot_tab[slot_row], g_slot_tab[row].word, word);
        if (-1 == ret)
        {
            return 0;
        }
...
void place_word(slot_t b, const char *word, int word_used_index, int slot_row)
{
    ...
    g_slot_tab[slot_row].word = &g_input_words[word_used_index][0];
            ...
            if (input[row][col] == 'X')
            {
                if (counting == 1)
                {
                    if (end_col - start_col > 1)
                    {
                        ...
                        g_slot_tab[g_slot_tab_index].s.col = start_col + 1;
        ...
        for (int row = 0; row < N; ++row)
        {
            if (input[row][col] == 'X')
            {
                if (counting == 1)
                {
                    ...
                    {
                        //register
                        g_slot_tab[g_slot_tab_index].s.row = start_row + 1;
...
int get_input()
{
    ...
    fs = fopen("crossword.txt", "r");

    for (int row = 0; row < N; ++row)
    {
        for (int col = 0; col < N + 1; ++col)
        {
            fscanf(fs, "%c", &g_common_input[row][col]);
    ...
    ls = fopen("words.txt", "r");

    while ((c = getc(ls)) && c != EOF)
    {
        if (c != '\n')
        {
    ...
    qsort(g_input_words, row, sizeof(g_input_words[0]), cmplen);
...
{
    FILE *out = NULL;
    out = fopen("crossword-solved.txt", "w");
    
    for (int row = 0; row < N; ++row)
    {
        for (int col = 0; col < N; ++col)
        {
            fprintf(out, "%c", g_common_input[row][col]);
        }
